...
def rank_algorithm(anomalies):
    ...
    for a in anomalies:
        ...
        inner_score = (
            ...
            + (
                transaction_map.get("is_transaction_t", 0)
                if a["transaction_id"]
                else transaction_map.get("is_transaction_f", 0)
            )
            + time_score
        )
...
def A1(atm_id, error_seen, max_timeout, kafka_offline, terminal_timeout):
    ...
    operational_impact = (
        "ATM unavailable for customer transactions due to network connectivity failure."
    )
...
def build_detailed_table(anomalies):
    ...
    for a in anomalies:
        ...
        if A_code == "A1":
            ...
            # checklist
            error_seen, max_timeout, kafka_offline, terminal_timeout = (
                False,
                0,
                False,
                False,
            ...
            explanation, operation_impact, recommendation = A1(
                atm_id, error_seen, max_timeout, kafka_offline, terminal_timeout
            )
            ...
            detailed_rows.append(
                {
                    ...
                    "detection_source": exp.get("source")
            ...
            # checklist
            low_count, empty_count, out_of_service, dispense_error, zero_tps = (
                0,
                0,
                False,
                False,
            ...
            explanation, operation_impact, recommendation = A2(
                atm_id, low_count, empty_count, out_of_service, dispense_error, zero_tps
            )
            ...
            # checklist
            mem_start, mem_end, gc_start, gc_end, high_cpu, oom_seen = (
                0,
                0,
                0,
                0,
            ...
            if source in ("ML_ENSEMBLE", "CLASSIFIER"):
                ...
                explanation, operation_impact, recommendation = (
                    _build_classifier_description(A_code, atm_id, confidence, if_score)
                )
            else:
                ...
                oom_seen = (
                    exp.get("oom_seen", False)
                    or exp.get("xgb_predicted") == "OutOfMemoryError"
                )

                explanation, operation_impact, recommendation = A3(
                    mem_start, mem_end, gc_start, gc_end, high_cpu, oom_seen
                )
            ...
            if source in ("ML_ENSEMBLE", "CLASSIFIER"):
                ...
                explanation, operation_impact, recommendation = A4(
                    max_restart, fatal_count, startup_count
                )
            ...
            if source in ("ML_ENSEMBLE", "CLASSIFIER"):
                ...
                explanation, operation_impact, recommendation = A5(
                    atm_id, max_rt5, min_success, max_failures, timeout_seen
                )
            ...
            # checklist
            mem_start, mem_max, cpu_max, net_error_max, timeout_seen = (
                0,
                0,
                0,
                0,
            ...
            if source in ("ML_ENSEMBLE", "CLASSIFIER"):
                ...
                explanation, operation_impact, recommendation = A6(
                    atm_id, mem_start, mem_max, cpu_max, net_error_max, timeout_seen
                )
            ...
            if source in ("ML_ENSEMBLE", "CLASSIFIER"):
                ...
                explanation, operation_impact, recommendation = A7(
                    atm_id, missing_field_count, malformed_metric, ooo
                )
