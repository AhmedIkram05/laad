...
def generate_otp_token(
    ...
    # OTP tokens only grant temporary USER access.
    token_role = "user"
    ...
    link = f"http://localhost:5173/dashboard?token={token}"
    logger.info(f"OTP generated by '{current_user['sub']}' for role={token_role} (requested={request.role})")
    return {
        "token": token,
        "link": link,
        "role": token_role,
