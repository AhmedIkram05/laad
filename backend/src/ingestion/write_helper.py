...
def write_batch(conn: psycopg2.extensions.connection, sql: str, rows: Iterable[tuple],
    ...
    while True:
        try:
            ...
            msg = str(e).lower()
            transient = any(k in msg for k in ("deadlock", "could not serialize", "could not obtain lock", "canceling statement due to user request", "lock"))
            if transient and attempt < retries:
                ...
                logger.warning('Transient DB error during write; retrying in %.3fs (attempt %d): %s', wait, attempt + 1, e)
                time.sleep(wait)
                attempt += 1
                continue
