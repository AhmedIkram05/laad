...
class RAGQueryRequest(BaseModel):
    ...
    enable_reflexion: Optional[bool] = Field(None, description="Enable self-critique and regeneration")
    enable_citation_grounding: Optional[bool] = Field(None, description="Enable entity citation verification")
    enable_self_consistency: Optional[bool] = Field(None, description="Enable multi-sample consistency scoring")
...
class RAGQueryResponse(BaseModel):
    ...
    self_consistency_score: Optional[float] = Field(None, description="Consistency across multiple samples (0-1)")
    verbalized_confidence: Optional[float] = Field(None, description="LLM's self-rated confidence (0-1)")
    grounding_score: Optional[float] = Field(None, description="Fraction of cited entities verified in sources (0-1)")
    generation_variance: Optional[float] = Field(None, description="1 - self_consistency_score")
    cross_encoder_used: bool = Field(False, description="Whether cross-encoder reranking was applied")
    was_revised: bool = Field(False, description="Whether reflexion triggered a revision")
