...
def emit_tick(producer, t: datetime, anomaly_last_seen: dict[str, datetime], backfill_mode: bool = False, backfill_prob: float = 0.01) -> None:
...
def backfill(producer, minutes: int) -> None:
    ...
    backfill_prob = 0.01
    log.info("Anomaly injection during backfill: %.4f (live: %.4f)", backfill_prob, ANOMALY_PROB)
    ...
    while t < now_utc() and not _shutdown_requested:
        try:
            emit_tick(producer, t, anomaly_last, backfill_mode=True, backfill_prob=backfill_prob)
        except Exception as exc:
    ...
    _in_backfill = False
    log.info("Backfill complete (%d ticks, live anomaly prob: %.4f)", count, ANOMALY_PROB)
