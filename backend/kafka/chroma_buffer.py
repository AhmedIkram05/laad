...
class ChromaBuffer:
    ...
    def _init(self) -> None:
        try:
            ...
            try:
                ...
                log.warning(
                    "Ollama embeddings unavailable, using simple chunking fallback: %s",
                    embed_exc,
                )
                self._embeddings = None
                self._chunker = None
    ...
    def _flush_atm(self, atm_id: str) -> None:
        ...
        try:
            ...
            anomaly_tags = [
                e.get("anomaly_tag") for e in events if e.get("anomaly_tag")
            ]
            dominant_severity = (
                self._get_dominant_severity(severities) if severities else None
            )
            dominant_anomaly = (
                max(set(anomaly_tags), key=anomaly_tags.count) if anomaly_tags else None
            )

            text_with_prefix = "\n".join(f"ATM: {atm_id} | {e['text']}" for e in events)
            ...
            log.debug(
                "Flushed %d chunks to ChromaDB for %s (severity=%s, anomaly_tag=%s)",
                len(documents),
                atm_id,
                dominant_severity,
                dominant_anomaly,
