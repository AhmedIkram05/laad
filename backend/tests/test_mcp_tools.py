...
class TestSearchKnowledge:
    def test_search_knowledge_shape(self, monkeypatch):
        ...
        class _FakeRetriever:
            ...
            def retrieve(self, query, atm_id=None, top_k=None, anomaly_type=None,
                         temporal_boost=True, error_only=None, most_recent_first=None):
                return [RetrievedChunk(
                    text="2026-05-15T10:00:00Z [ATM_APP] NETWORK_DISCONNECT: ATM-GB-0001 connection lost",
                    chunk_id="chunk-1", atm_id="ATM-GB-0001",
                    timestamp="2026-05-15T10:00:00Z", distance=0.1, confidence_score=0.9,
                )]

        monkeypatch.setattr("backend.src.mcp.tools.vector.get_retriever", lambda: _FakeRetriever())

        async def _go():
            async with create_connected_server_and_client_session(mcp._mcp_server) as session:
                return await _call_tool(session, "search_knowledge", {
                    "query": "network disconnect", "atm_id": "ATM-GB-0001",
                })
    ...
    def test_search_knowledge_store_unavailable(self, monkeypatch):
        ...
        async def _go():
            async with create_connected_server_and_client_session(mcp._mcp_server) as session:
                return await _call_tool(session, "search_knowledge", {"query": "anything"})
...
class TestSqlTools:
    def test_query_anomalies_empty_shape(self):
        async def _go():
            async with create_connected_server_and_client_session(mcp._mcp_server) as session:
                return await _call_tool(session, "query_anomalies", {"atm_id": "ATM-GB-0001"})
    ...
    def test_get_atm_info_seeded(self):
        async def _go():
            async with create_connected_server_and_client_session(mcp._mcp_server) as session:
                return await _call_tool(session, "get_atm_info", {"atm_id": "ATM-GB-0001"})
    ...
    def test_get_atm_info_unknown(self):
        async def _go():
            async with create_connected_server_and_client_session(mcp._mcp_server) as session:
                return await _call_tool(session, "get_atm_info", {"atm_id": "ATM-NOPE-9999"})
    ...
    def test_get_error_context_requires_one_id(self):
        ...
        out = _run(_go())
        assert "exactly one of correlation_id or transaction_id required" in out["error"]
    ...
    def test_get_anomaly_class_info_known(self):
        async def _go():
            async with create_connected_server_and_client_session(mcp._mcp_server) as session:
                return await _call_tool(session, "get_anomaly_class_info", {"anomaly_class": "a3"})
    ...
    def test_get_anomaly_class_info_unknown(self):
        async def _go():
            async with create_connected_server_and_client_session(mcp._mcp_server) as session:
                return await _call_tool(session, "get_anomaly_class_info", {"anomaly_class": "A9"})

        out = _run(_go())
        assert "unknown anomaly class" in out["error"]

    def test_get_rag_collection_stats_unavailable(self, monkeypatch):
        monkeypatch.setattr("backend.src.mcp.tools.knowledge.get_retriever", lambda: None)
