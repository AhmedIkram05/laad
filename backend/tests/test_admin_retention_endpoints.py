...
def test_admin_retention_get_put_and_permissions():
    ...
    try:
        with TestClient(app) as client:
            ...
            bad = client.put(
                "/admin/retention", json={"days": 2}, headers=admin_headers
            )
            assert bad.status_code == 400

            reg = client.post(
                "/auth/register", json={"username": "normal1", "password": "password1"}
            )
            assert reg.status_code == 201
            login = client.post(
                "/auth/login", data={"username": "normal1", "password": "password1"}
            )
            ...
            forbidden = client.put(
                "/admin/retention", json={"days": 7}, headers=user_headers
            )
...
def test_admin_wipe_invokes_run_wipe_and_requires_admin(monkeypatch):
    ...
    try:
        with TestClient(app) as client:
            ...
            reg = client.post(
                "/auth/register", json={"username": "normal2", "password": "password2"}
            )
            assert reg.status_code == 201
            login = client.post(
                "/auth/login", data={"username": "normal2", "password": "password2"}
            )
            assert login.status_code == 200
            user_token = login.json()["access_token"]
            user_headers = {"Authorization": f"Bearer {user_token}"}
