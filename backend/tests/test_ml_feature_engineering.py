...
class TestExtractFeatures:
    def test_returns_correct_shape(self):
        ...
        assert feat.shape == (len(FEATURE_NAMES),), (
            f"Expected {len(FEATURE_NAMES)} features, got {feat.shape}"
        )
    ...
    def test_error_counts_per_source(self):
        ...
        assert feat[idx_err] == 2.0, (
            f"atm_app_error_count should be 2, got {feat[idx_err]}"
        )
        assert feat[idx_fatal] == 1.0, (
            f"terminal_handler_fatal_count should be 1, got {feat[idx_fatal]}"
        )
        assert feat[idx_oom] == 1.0, (
            f"terminal_handler_oom_count should be 1, got {feat[idx_oom]}"
        )
        assert feat[idx_has_oom] == 1.0, (
            f"has_oom_event should be 1, got {feat[idx_has_oom]}"
        )
    ...
    def test_cassette_empty_count(self):
        ...
        assert feat[idx] == 1.0, (
            f"hardware_cassette_empty_count should be 1, got {feat[idx]}"
        )
...
class TestExtractLabel:
    ...
    def test_invalid_tag_format_ignored(self):
        rows = [
            {"raw_payload": {"_anomaly_tag": "B1"}},
            {"raw_payload": {"_anomaly_tag": "A9"}},
        ]
        assert extract_label(rows) is None
...
class TestFeatureNames:
    ...
    def test_all_features_named(self):
        assert len(FEATURE_NAMES) == FEATURE_COUNT, (
            f"Expected {FEATURE_COUNT} features, got {len(FEATURE_NAMES)}"
        )
        assert all(isinstance(f, str) for f in FEATURE_NAMES), (
            "All FEATURE_NAMES must be strings"
        )
