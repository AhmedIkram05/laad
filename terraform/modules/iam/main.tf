...
data "aws_iam_policy_document" "github_actions_terraform_readonly" {
  statement {
    ...
    actions = [
      ...
      "ec2:Describe*",
      # StopInstances/StartInstances/ModifyInstanceAttribute needed for Terraform
      # to update user_data on existing EC2 instances (e.g. Kafka) by stopping,
      # modifying user data attribute, then starting.
